	...
	public function checkIsolate(){
		...
		if(this.betaFlags.has(VendorBetaFlags.Cagette2)){
			...
			if ( noCagette2Groups.length>0 ){
				...
				throw sugoi.ControllerAction.ControllerAction.ErrorAction("/user/choose",'Le producteur "${this.name}" a l\'option Cagette2 activée et ne peut pas fonctionner avec des groupes qui n\'ont pas activé cette option ($name). Contactez nous sur <b>::theme.supportEmail::</b> pour régler le problème.');
			}
